...
namespace tigl
{
	...
	private:
		GLuint id;
		unsigned int size;
		friend void drawVertices(GLenum shape, std::shared_ptr<VBO> vbo);
		friend void deleteVbo(std::shared_ptr<VBO> vbo);
		friend std::shared_ptr<VBO> createVbo(const std::vector<Vertex>& vertices);
	};
	...
	// Creates a VBO
	std::shared_ptr<VBO> createVbo(const std::vector<Vertex>& vertices);

	// Deletes a VBO
	void deleteVbo(std::shared_ptr<VBO> vbo);

	// draws vertices from a VBO
	void drawVertices(GLenum shape, std::shared_ptr<VBO> vbo);
}
